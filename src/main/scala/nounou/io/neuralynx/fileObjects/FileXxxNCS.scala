...
/**
  ...
  */
abstract class FileReadNCS(file: File) extends FileReadNeuralynx(file) with FileNCS with NNDataChannel with NNDataChannelNumbered  {

  override lazy val header: NNHeaderNCS = new NNHeaderNCS( originalHeaderText )
